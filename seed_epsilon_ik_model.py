...
class SeedEpsilonIKModel(nn.Module):
    def __init__(self, device, config: SeedEpsilonIKConfig):
        ...
        # self.kan = KAN([12 + 7 + 7 + 7 + 336] + [config.d_model] * 6 + [7, ])
    ...
    def forward(self, pose, seed, max_diff) -> torch.Tensor:
        ...
        x = self.fc_joints(x11 + x12)
        # x = self.kan(x)

        # pose_R, pose_t = pose[:, :9].view(-1, 3, 3), pose[:, 9:].view(-1, 3)
        # x = self.affine_loss(self.fk(seed), (pose_R, pose_t)) * x / torch.norm(x, dim=-1, keepdim=True) / 2
        x = max_diff * x / torch.norm(x, dim=-1, keepdim=True)
        x = self.clamp(seed + x) - seed
        return x

    def find_ik(self, pose, seed, n_times=100):
        """
        :param pose: shape (batch_size, 12)
        :param seed: shape (batch_size, 7)
        :return:
        ...
        for _ in range(n_times):
            delta = self(pose, seed)
            seed = seed + delta
            loss = loss_affine.loss_fn(self.fk(seed), (pose[:, :9].view(-1, 3, 3), pose[:, 9:].view(-1, 3)))
            prediction_losses.append(loss)
        prediction_losses = torch.stack(prediction_losses, dim=-1)
        min_prediction_loss, min_prediction_loss_index = torch.min(prediction_losses, dim=-1)
        mean_min_prediction_loss = torch.mean(min_prediction_loss)
        print(f"Mean min prediction loss: {mean_min_prediction_loss.item()}")
        return seed
