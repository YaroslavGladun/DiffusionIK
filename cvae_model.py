...
class CVAEDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = torch.relu(self.fc3(x2 + x1))
        x4 = torch.relu(self.fc4(x3 + x2))
        x5 = torch.sigmoid(self.fc5(x4 + x3))
        return x5


class CVAE(nn.Module):
    def __init__(self, device, input_size, output_size, hidden_size, condition_size):
        super(CVAE, self).__init__()
        self.joint_scaler = JointValuesScaler(device)
        self.joint_scaler_inverse = JointValuesScalerInverse(device)
